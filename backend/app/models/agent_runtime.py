...
class GuardrailViolation(TimestampMixin, Base):
    ...
    event_signal_id: Mapped[str | None] = mapped_column(String(120), index=True)
    event_id: Mapped[str | None] = mapped_column(String(120), index=True)
    confidence: Mapped[float | None] = mapped_column(Float)
    matched_by_json: Mapped[list[str] | None] = mapped_column(JSON)
    match_features_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    guardrail_flags_json: Mapped[list[str] | None] = mapped_column(JSON)
    source_signal_ids_json: Mapped[list[str] | None] = mapped_column(JSON)
