...
- Day 38 第一版不写业务表；后续可写入事件信号中间表或抽取审计日志。
...
- embedding 高相似但缺少实体、动作或时间支撑时，最多进入中置信度复核。
...
- 综合 ID / URL、标题包含、BM25 / n-gram、embedding、实体、动作、时间窗口和来源证据进行候选重排；对象约束留作后续扩展。
